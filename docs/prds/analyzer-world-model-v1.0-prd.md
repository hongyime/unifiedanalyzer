...
- [ ] Redesign person overview with accounts, tiers, confidence, latest activity, and evidence summary.
...
- Person pages now include a Media/Faces tab backed by `/api/entities/{entity_id}/media-faces`, with owner and person-in-photo confirm/reject actions wired to durable media decision events.
