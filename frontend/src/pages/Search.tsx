...
export default function SearchPage() {
  ...
  return (
    <div>
      ...
      <form onSubmit={submit} className="sticky top-0 z-10 mb-4 flex flex-wrap items-center gap-2 bg-background/85 py-2 backdrop-blur">
        <div className="relative min-w-[260px] flex-1">
          <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-text-muted" />
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
            className="w-full rounded-lg border border-border-strong bg-surface-2 py-2.5 pl-9 pr-3 text-sm outline-none transition-colors focus:border-accent"
        ...
        <div className="space-y-2">
          ...
          {result.results.map((row) => (
            <a
              ...
              className="block rounded-lg border border-border-strong bg-surface-2 p-3 transition-colors hover:border-white/15 hover:bg-hover"
            >
              <div className="mb-1 flex flex-wrap items-center gap-2 text-xs text-text-muted">
                <span className="rounded bg-hover px-1.5 py-0.5 font-medium uppercase tracking-wide text-text-secondary">{row.platform}</span>
                <span>{row.occurred_at ? new Date(row.occurred_at).toLocaleString() : '-'}</span>
                <span className="mono rounded bg-hover px-1.5 py-0.5 text-text-secondary">score {row.score.toFixed(4)}</span>
